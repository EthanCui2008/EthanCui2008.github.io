...
const formatKind = (value: string): string => value.charAt(0).toUpperCase() + value.slice(1);

const blogState = (dateLabel: string): string =>
  dateLabel.toLowerCase() === "draft" ? "Draft" : "Published";

const projectState = (statusLabel: string | undefined): string => normalizeLabel(statusLabel, "Active");

const normalizeBlogDate = (value: string | undefined): string => {
...
const sampleGradient = (colors: string[], t: number): string => {
...
const windowedSegments = (
  palette: string[],
  start: number,
  span: number,
  segments = 12,
): string[] => {
  const source = palette.length ? palette : MASTER_PALETTE;
...
const updatePaletteBar = (pathname: string, blogs: BlogItem[], projects: ProjectItem[]): void => {
  const bar = document.querySelector<HTMLDivElement>("#sidebar-palette");
  if (!bar) return;
  ...
  if (slug) {
    ...
    const key = projects[0]?.title || "projects";
    palette = projects[0]?.palette?.length ? projects[0].palette : MASTER_PALETTE;
  ...
  colors.forEach((color, index) => {
    const cell = bar.children[index] as HTMLElement;
    if (cell) {
...
const renderBlogList = (blogs: BlogItem[]): void => {
  const root = document.querySelector<HTMLDivElement>("#blog-list");
  if (!root) {
    return;
  }
...
const renderProjectList = (projects: ProjectItem[]): void => {
  const root = document.querySelector<HTMLDivElement>("#project-list");
  if (!root) {
    return;
  }
...
const initData = async (): Promise<{ blogs: BlogItem[]; projects: ProjectItem[] }> => {
...
const setActiveNav = (section: string): void => {
  document.querySelectorAll<HTMLAnchorElement>(".page-nav-btn").forEach((btn) => {
...
const sectionFromPath = (pathname: string): "home" | "blog" | "projects" => {
  if (pathname.startsWith("/blog")) return "blog";
  if (pathname.startsWith("/projects")) return "projects";
  return "home";
};

const swapMain = async (url: string, push = true): Promise<void> => {
  try {
...
const setupSpaNav = (): void => {
  const navLinks = document.querySelectorAll<HTMLAnchorElement>(".page-nav-btn");
